...
//Stuff to make shadows work with 3dsMax//////
#include <shadowMap.fxh> 

SHADOW_FUNCTOR(shadowCaster1);
SHADOW_FUNCTOR(shadowCaster2);

float makeShadow(float4 wp, int passNum)
{
	float shadVal = 1.0;
	if (passNum == 1) shadVal = shadowCaster1(wp);
	else if (passNum == 2) shadVal = shadowCaster2(wp);
	return shadVal;
}
...
PSOutput PS (PSInput PSIn, uniform half4 lightColIn, uniform int numPass) : COLOR
{
	...
	half shadow = makeShadow(PSIn.UV1, numPass); //Max-specific shadows
	//shadow += 0.5;
	diffCol.rgb *= shadow;
